...
Claude Conductor (claude --chrome --dangerously-skip-permissions)
  ...
  │    └─► claude -p worker → report .md + PDF + DB write
  │
  ├─ Posting 2: click next, reads JD + URL
  │    └─► claude -p worker → report .md + PDF + DB write
  │
  └─ End: ping dashboard + summary
```
...
4. **For each pending URL**:
   a. Chrome: click on the posting → read JD text from DOM
...
6. **End**: Ping dashboard (`npx tsx dashboard/scripts/db-write.ts ping`) + summary
...
- Each worker is independent: failure on posting #47 does not affect the others
...
2. PDF in `output/`
3. DB write via `npx tsx dashboard/scripts/db-write.ts` (job + eval + ping)
